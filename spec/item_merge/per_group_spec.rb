...
describe 'Poloxy::ItemMerge::PerGroup' do
  ...
  describe "#merge_into_messages called by #{klass_full}" do
    ...
    context 'When some items are snoozed' do
      ...
      describe 'Merged container' do
        it 'has messages of all groups but snoozed' do
          msgs = @mc.messages
          expect(msgs.length).to eq( 2*2 - 1 )
          expect( msgs.map(&:item) ).to eq( 1.upto(2*2 - 2).map { Poloxy::MERGED_ITEM }.push('item2.2.1') )
          expect( msgs.map(&:level) ).to eq( 1.upto(2*2 - 2).map { 2 }.push(1) )
          groups_expected = @items.map(&:group).uniq
          groups_expected.delete('group1.1')
          expect( msgs.map(&:group) ).to eq groups_expected
        end
